...
│  │  └──────────┘                          │     │
│  │                                         │     │
│  │  5개 도메인 서비스 모두 단일 노드에 배포 │     │
│  └────────────────────────────────────────┘     │
└──────────────────────────────────────────────────┘

* 현재 구성: Worker Node 1개에 5개 도메인 서비스 배포
* 향후 확장: 트래픽 증가 시 Worker Node 추가 가능
```
...
[Kubernetes - 목표]
kubectl apply -f k8s/
→ 단일 Worker Node에서 여러 Pod로 분산 실행
→ 스케일링: kubectl scale deployment product --replicas=3 (자동화 가능)
→ 장애 복구: Pod 죽으면 자동 재시작
→ 향후 확장: 트래픽 증가 시 Worker Node 추가로 수평 확장
```
