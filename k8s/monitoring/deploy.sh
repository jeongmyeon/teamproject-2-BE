...
echo "   Username: admin"
echo "   Password: \${GRAFANA_ADMIN_PASSWORD:-admin1234} (기본값, 프로덕션에서는 반드시 변경)"
echo ""
echo "   ⚠️  보안 강화: 프로덕션 배포 시 비밀번호 변경 필수!"
echo "   kubectl create secret generic grafana-secrets \\"
echo "     --from-literal=admin-password='YOUR_STRONG_PASSWORD' \\"
echo "     -n biddy-monitoring --dry-run=client -o yaml | kubectl apply -f -"
echo "   kubectl rollout restart deployment/grafana -n biddy-monitoring"
echo ""
